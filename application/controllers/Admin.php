...
class Admin extends CI_Controller {
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('cat_list',$page_data);
			...
			$this->load->view('edit_cat',$page_data);
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('sub_cat_list',$page_data);
			...
			$this->load->view('edit_sub_cat',$page_data);
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('brand_list',$page_data);
			...
			$this->load->view('edit_brand',$page_data);
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('system_setup',$page_data);
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('product_list',$page_data);
			...
			$page_data['sdata'] = $this->db->get_where('ecomm_products',array('product_code'=>$product_code))->row();

			$page_data['cats'] = $this->crud->get_all('category');

			//page_data['current_brand'] = $this->crud->get_name_by_id('brands','name',$row->id);

			$page_data['brands'] = $this->crud->get_all('brands');

			$this->load->view('edit_product',$page_data);
			...
			$data = array(
				...
				'cat_id'	=>	$input_data['cat_id'],
				'sub_cat_id'	=>	$input_data['sub_cat_id'],
				'brand_id'	=>	$input_data['brand_id'],
				'product_overview'	=>	$input_data['product_overview'],
				'description'	=>	$input_data['description'],	
				'old_price'	=>	$input_data['old_price'],			
				'price'	=>	$input_data['price'],
				'new_arrival'	=>	$input_data['new_arrival'],
				'offer'	=>	$input_data['discount'],
				'percentance'	=>	$input_data['discount_percentance'],
				'free_product_name'	=>	$input_data['free_product_name'],
				'free_product_short_description'	=>	$input_data['free_product_short_desc'],
				'product_status'	=>	$input_data['product_status']
			...
			$this->crud->update_product('ecomm_products',$_GET['product_code'], $data);
			...
			$this->crud->save_data('ecomm_products',$data);
			...
			$this->crud->delete_product('ecomm_products',$_GET['id']);
...

	{
		...
		if(!isset($_GET['a'])) {
			...
			$this->load->view('all_images',$page_data);
			...
			$this->load->view('edit_slider_image',$page_data);
